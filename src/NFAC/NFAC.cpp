...
void Loki::NoFollowerAttackCollision::InstallMeleeHook() {
    REL::Relocation<std::uintptr_t> MeleeHook{RELOCATION_ID(37650, 38603)};  //+38B

    auto& trmp = SKSE::GetTrampoline();
    _MeleeFunction = trmp.write_call<5>(MeleeHook.address() + REL::VariantOffset(0x38B, 0x45A, 0x38B).offset(), MeleeFunction);

    logger::info("Unk Hook injected");
}

void Loki::NoFollowerAttackCollision::InstallSweepHook() {
    REL::Relocation<std::uintptr_t> SweepHook{RELOCATION_ID(37689, 38603)};  //+DD

    auto& trmp = SKSE::GetTrampoline();
    _SweepFunction = trmp.write_call<5>(SweepHook.address() + REL::VariantOffset(0xDD, 0x40A, 0xDD).offset(), SweepFunction);

    logger::info("Unk Hook 2 injected");
}

void Loki::NoFollowerAttackCollision::InstallArrowHook() {
    REL::Relocation<std::uintptr_t> arrowHook{RELOCATION_ID(43027, 44218)};  //+90
...
void Loki::NoFollowerAttackCollision::MeleeFunction(RE::Character* a_aggressor, RE::Actor* a_victim, std::int64_t a3,
                                                    char a4, float a5) {
    if (!a_victim || !a_aggressor || !toggle) {
        return _MeleeFunction(a_aggressor, a_victim, a3, a4, a5);
    }

    if ((a_aggressor->IsPlayerRef() || a_aggressor->IsPlayerTeammate()) && a_victim->IsPlayerTeammate() ||
        (a_victim->IsGuard() && !a_victim->IsHostileToActor(a_aggressor))) {
        return;
    }
...
void Loki::NoFollowerAttackCollision::SweepFunction(RE::Character* a_aggressor, RE::Actor* a_victim, std::int64_t a3,
                                                    char a4, float a5) {
    if (!a_victim || !a_aggressor || !toggle) {
...
void Loki::NoFollowerAttackCollision::ArrowFunction(RE::Character* a_aggressor, RE::Actor* a_victim, std::int64_t a3,
                                                    char a4, float a5) {
    if (!a_victim || !a_aggressor || !toggle) {
...
RE::BSEventNotifyControl Loki::OnInput::ProcessEvent(RE::InputEvent* const* a_event,
